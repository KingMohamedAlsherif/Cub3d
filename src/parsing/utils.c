...
int	is_color(t_game *cub, char *line, int *color_idx, int *color_arr)
{
	if (!line || !*line)
		return (0);
...
void	is_player(t_game *cub, char *map_line, int y)
{
	t_map	*map;
	int		x;
...
char	*set_map_line(t_game *cub, t_map *map, char *map_line)
{
	int		len;
	int		diff;
	char	*tmp;
...
/*
	*	is_color(t_game *cub, char *line, int *color_idx, int *color_arr)
	{
		We use this function to parse the color from the line.
		
		Parameters:
			t_game *cub - the main struct of the game.
	...
	*	is_player(t_game *cub, char *map_line)
	{
	...
	*	set_map_line(t_game *cub, t_file *file, char *map_line)
	{
